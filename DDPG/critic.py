...
class Critic:
    ...
    def __init__(self, inp_dim, out_dim, lr, tau):
        ...
        self.model.compile(tf.keras.optimizers.Adam(self.lr), 'mse')
        self.target_model.compile(tf.keras.optimizers.Adam(self.lr), 'mse')
    ...
    def network(self):
        ...
        """
        state = tf.keras.layers.Input((self.env_dim))
        action = tf.keras.layers.Input((self.act_dim,))
        x = tf.keras.layers.Dense(256, activation='relu')(state)
        x = tf.keras.layers.concatenate([Flatten()(x), action])
        x = tf.keras.layers.Dense(128, activation='relu')(x)
        out = tf.keras.layers.Dense(1, activation='linear', kernel_initializer=tf.random_uniform_initializer())(x)
